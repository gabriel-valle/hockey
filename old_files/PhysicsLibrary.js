var World = function (ctx, canvas)
{
    ...
    {
        for(let counter = 0; counter<this.objects.length-1; counter++)
        {
            for(let scounter = counter+1; scounter<this.objects.length; scounter++)
            {
                if(this.objects[counter].getLayer() == this.objects[scounter].getLayer())
                {
                    ...
                    if(col)
                    {
                        ...
                        var vel1f = vel1*(mass1 - mass2)/(mass1 + mass2) + vel2*(2*mass2/(mass1 + mass2));
                        var vel2f = vel2*(mass2 - mass1)/(mass2 + mass1) + vel1*(2*mass1/(mass2 + mass1));
                        ...
                        //tela.objects[counter].setVelByAngle(20, (tela.objects[counter].getInclinacao(col)+90)%360);
                        //tela.objects[scounter].setVelByAngle(20, (tela.objects[scounter].getInclinacao(col)+90)%360);
                        console.log('pos - ' + JSON.stringify(tela.objects[counter].position) + ', vel - ' + JSON.stringify(tela.objects[counter].vel));
                        console.log('pos - ' + JSON.stringify(tela.objects[scounter].position) + ', vel - ' + JSON.stringify(tela.objects[scounter].vel));
                    }
...
var Retangulo = function(newX, newY, largura, altura)
{
    ...
    this.draw = function(context, preencher)
    {
        if(typeof(preencher)=="undefined")
            preencher = true;
        context.beginPath();
        if(this.getImageIndex() >= 0 && tela.images[this.getImageIndex()]["loaded"] == true)
        {
            if(this.filter)
            {
                context.fillStyle = this.color.getColor();
